...
import { createRefreshToken, findRefreshTokenByRaw, rotateRefreshToken, revokeRefreshTokenByRaw, hashToken } from "../../common/utils/token";
...
export const register = async (req: Request, res: Response) => {
    try {
        ...
        const hashPassword = validatedData.password ? await bcrypt.hash(validatedData.password, 10) : undefined
...
export const login = async (req: Request, res: Response) => {
    try {
        ...
        if (!user || !user.password) return res.status(httpStatus.UNAUTHORIZED).json({ error: "Invalid credentials" });

        const ok = await bcrypt.compare(validatedData.password, user?.password);
        if (!ok) return res.status(httpStatus.UNAUTHORIZED).json({ error: "email or password is incorrect" });
...
export const verifyEmail = async (req: Request, res: Response) => {
    try {
        ...
        if (!token) return res.status(httpStatus.BAD_REQUEST).json({ message: "Missing token ,Please try again later" });
        ...
        if (record.expiresAt < new Date()) return res.status(httpStatus.BAD_REQUEST).json({ message: "Token expired" });

        await prisma.user.update({ where: { id: record.userId }, data: { isEmailVerified: true } });
        await prisma.emailVerification.delete({ where: { id: record.id } });
        res.status(httpStatus.OK).sendFile(path.join(__dirname, "../../common/view/verify-success.html"));
    } catch (error) {
