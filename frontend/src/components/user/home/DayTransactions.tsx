...
import { MdSubject, MdOutlineNotInterested } from 'react-icons/md';
...
const DayTransactions: React.FC<DayTransactionsProps> = ({
  ...
  const [localGrouped, setLocalGrouped] = useState<Record<number, Transaction[]>>({});
  const groupedDataString = JSON.stringify(groupedByDate);

  useEffect(() => {
    setLocalGrouped(groupedByDate);
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    if (source.droppableId !== destination.droppableId) {
      showToastAlert('จัดลำดับได้เฉพาะในวันเดียวกันเท่านั้นครับ', 'warning');
      return;
    }

    const dateKey = Number(source.droppableId);
    const newItems = Array.from(localGrouped[dateKey]);
    ...
    try {
      await axios.put('/transaction/reorder', { items: payload });
    } catch (error) {
      console.error('Reorder failed:', error);
      showToastAlert('ไม่สามารถจัดลำดับได้ กรุณาลองใหม่', 'error');
      setLocalGrouped(groupedByDate);
    }
  };
    ...
    <DragDropContext onDragEnd={handleDragEnd}>
      <div className="w-5/6 md:min-w-[550px] md:w-[50%]">
        {sortedDates.map((date, index) => {
          const dayTransactions = localGrouped[date] || [];
              ...
              <Droppable droppableId={String(date)}>
                {(provided) => (
                  <div
                    ...
                    className={`bg-blue-100 dark:bg-blue-900/20 ${isLastDate ? 'rounded-b-md' : ''}`}
                  >
                    {dayTransactions.map((transaction, tIndex) => {
                      ...
                      return (
                        <Draggable
                          ...
                          draggableId={String(transaction.id)}
                          index={tIndex}
                        >
                          {(provided, snapshot) => (
                            <div
                              ...
                              className={`flex items-center justify-between px-4 h-16 cursor-grab active:cursor-grabbing
                                ${
                                  snapshot.isDragging
                                    ? 'bg-white shadow-xl scale-[1.02] z-50 rounded-lg ring-2 ring-blue-400 dark:bg-black-700'
                                    : 'hover:bg-blue-200/50 dark:hover:bg-blue-800/30'
                                }
                                ${shouldRoundBottom && !snapshot.isDragging ? 'rounded-b-md' : ''}
                              `}
