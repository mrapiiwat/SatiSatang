...
const FloatingBubble: React.FC<FloatingBubbleProps> = ({ onClick }) => {
  ...
  const snapToEdge = () => {
    ...
    const snapX = position.x + BUBBLE_SIZE / 2 > middle
  ...
  return (
    <div
      ...
      className={`fixed rounded-full bg-blue-500 text-white flex items-center justify-center select-none shadow-lg cursor-grab active:cursor-grabbing ${isSnapping ? "transition-all duration-300 ease-out" : ""}`}
      style={{
        width: BUBBLE_SIZE,
        height: BUBBLE_SIZE,
        left: position.x,
